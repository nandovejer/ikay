/**
 * Inspiration by Robin Linus in https://webkay.robinlinus.com
 * His work aweasome
 */
...
let socialNetworksLogged = [];
let socialNetworksFail = [];

export const checkSocialNetworks = () => {
	socialNetworks.forEach(function (network) {
		var img = document.createElement("img");
		img.src = network.url;
		img.onload = function () {
			socialNetworksLogged.push(network.name);
		};
		img.onerror = function () {
			socialNetworksFail.push(network.name);
		};
	});
};

export const detectLogged = () => {
	return new Promise((resolve, reject) => {
		resolve(checkSocialNetworks());
		reject(null);
	});
};

export default detectLogged;
